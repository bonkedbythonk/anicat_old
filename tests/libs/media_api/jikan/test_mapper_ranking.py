...
def test_fuzzy_promotes_strong_match():
    ...
    item2 = make_item(
        202, "Fullmetal Alchemist: Brotherhood", "Hagane no Renkinjutsushi"
    )

    api_response = {
        "data": [item1, item2],
        "pagination": {
            "items": {"total": 2, "per_page": 25},
            "current_page": 1,
            "has_next_page": False,
        },
    }
    ...
    assert (
        result.media[0].title.english and "Fullmetal" in result.media[0].title.english
    )
