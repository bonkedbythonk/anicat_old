...
def download_episodes(ctx: Context, state: State) -> State | InternalDirective:
    ...
    success_count, failed_episodes = ctx.download.download_episodes_sync(
        media_item, selected_episodes
    )

    if success_count == len(selected_episodes):
        feedback.success(
            f"Successfully downloaded all {len(selected_episodes)} episodes!"
        )
