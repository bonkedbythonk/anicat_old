...
from ...core.constants import LOCAL_API_PORT


@click.command()
@click.option("--port", default=LOCAL_API_PORT, help="Port to run the API on.")
@click.option(
    "--host",
    default="127.0.0.1",
    help="Host to bind the server to. Use 127.0.0.1 for maximum privacy.",
)
@click.option(
...
@click.option(
    "--reload",
    is_flag=True,
    help="Enable hot reload on code changes (development mode).",
)
@click.pass_obj
